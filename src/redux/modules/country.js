...
export const countryMainDB = createAsyncThunk(
  ...
  async (thunkAPI) => {
    try {
      const res = await axios.get('https://a-fo-back.shop/main/allCountry');
      return res.data;
    } catch (error) {
  ...
  async (countryName, thunkAPI) => {
    try {
      const res = await axios.get(
        `https://a-fo-back.shop/sub1/filtering/country?countryName=${countryName}`
      );
      return res.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(error);
...
export const CountrySub1DB = createAsyncThunk(
  ...
  async (data, thunkAPI) => {
    try {
      const res = await axios.get(
        `https://a-fo-back.shop/sub2/filtering/target?targetName1=${data.targetName1}&targetName2=${data.targetName2}&targetName3=${data.targetName3}&targetName4=${data.targetName4}&countryName=${data.countryName}`
      );
      return res.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(error);
