  ...
  async (purpose, thunkAPI) => {
    try {
      const res = await axios.get(
        `https://13.125.244.244/sub1/filtering/target?purpose=${purpose}`
      );
      return res.data;
    } catch (error) {
...
export const targetSub2DB = createAsyncThunk(
  ...
  async (data, thunkAPI) => {
    try {
      const res = await axios.get(
        `https://13.125.244.244/sub2/filtering/country?countryName1=${data.countryName1}&countryName2=${data.countryName2}&countryName3=${data.countryName3}&countryName4=${data.countryName4}&targetName=${data.targetName}`
      );
      return res.data;
    } catch (error) {
      return thunkAPI.rejectWithValue(error);
